...
const ProfileProduct = ({ title = 'Ожидает оплаты', subTitle = AppString.left, time = "23 : 50 : 33", buttonText = AppString.pay }) => {
  ...
  return <View style={styles.profileProduct}>

    <View style={{ flexDirection: 'row', width: '60%', alignItems: 'center' }}>
      <Image source={{ uri: imagesUrl.profile }} style={{ height: 44, width: 44, borderRadius: 4 }} />
      <View style={{ marginStart: 10 }}>
        <Text style={[styles.textStyle, { fontSize: 13, fontWeight: '500' }]}>{title}</Text>
        <Text style={[styles.textStyle, { color: colors.lightOrange }]}>{time} <Text style={[styles.textStyle, { color: colors.grey }]}>{subTitle}</Text></Text>
      </View>
...
const CommonButton = ({ text = AppString.pay, endColor = colors.endOrange,
  ...
  return <TouchableOpacity onPress={onClick}>
    <LinearGradient
      ...
      style={{ padding: 10, paddingVertical:6, borderRadius: 1000, marginEnd: 10 }}
    >
  ...
  return <View style={styles.profile}>
    <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
      ...
      <View style={{ paddingHorizontal: 10 }}>
        ...
        <Text style={[styles.textStyle, { color: colors.grey, fontSize: 14 }]}>Nickname92</Text>
